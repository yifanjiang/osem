class EventAttachmentsController < ApplicationController
  ...
  def show
    ...
    if organizer_or_admin? || current_user.conference_person == upload.event.submitter
      send_file upload.attachment.path
    else
  ...
  def create
    ...
    if !organizer_or_admin?
      begin
        event = current_user.conference_person.events.find(params[:proposal_id])
      rescue Exception => e
  ...
  def update
    @proposal = current_user.conference_person.events.find(params[:proposal_id])
  ...
  def destroy
    ...
    if organizer_or_admin? || current_user.conference_person == @proposal.submitter
